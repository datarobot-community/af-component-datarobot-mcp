...
from dr_mcp_clients.files_catalog_pulumi import FilesCatalogBundle, source_bundle_hash
from dr_mcp_clients.workload_api_client import build_artifact_from_image_uri
...
from ..{{mcp_app_name}}_user_params import MCP_USER_RUNTIME_PARAMETERS
...
DEFAULT_DOCKERFILE_RELATIVE_PATH = "docker/Dockerfile"
...
def _resolve_dockerfile_relative_path() -> str | None:
    ...
    Set ``WORKLOAD_DOCKERFILE_PATH=none`` (or ``false``/``0``) to force generated mode.
    """
...
def _resolve_container_port() -> int:
    return int(os.getenv("WORKLOAD_CONTAINER_PORT", str(WORKLOAD_CONTAINER_PORT)))
                    ...
                    "containers": [
                        {
                            ...
                            "resource_allocation": {
                                "cpu": float(os.getenv("WORKLOAD_CPU", "1")),
                                "memory": int(
                                    os.getenv(
                                        "WORKLOAD_MEMORY",
...
def _export_workload_endpoints(
    ...
    pulumi.export("workload_endpoint", workload.endpoint)
    pulumi.export("workload_id", workload.id)
    pulumi.export("workload_name", workload.name)
    pulumi.export("workload_artifact_id", workload.artifact_id)
...
def provision_workload_mcp_server(
    *,
    ...
    pulumi.export("files_catalog_id", catalog.catalog_id)
    pulumi.export("files_catalog_version_id", catalog.catalog_version_id)
    if execution_environment is not None:
        pulumi.export("workload_execution_environment_id", execution_environment.id)
        pulumi.export(
            "workload_execution_environment_version_id",
...
def provision_workload_mcp_server_from_image_uri(
    *,
    ...
    pulumi.export("workload_image_uri", workload_image_uri)
    mcp_workload_endpoint = _export_workload_endpoints(mcp_server_asset_name, workload)
