...
- Test MCP server with mock DataRobot client
...
- It can work with a real DataRobot client as well by setting `should_mock_dr_client` to `False`
```python
async with integration_test_mcp_session(should_mock_dr_client=False) as session
```
