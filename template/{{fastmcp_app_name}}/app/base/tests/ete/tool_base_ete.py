...
def _check_dict_has_keys(expected: dict, actual: dict, path: str = "") -> bool:
    """
    Recursively check if all keys in expected dict exist in actual dict.
    ...
    for key in expected:
        ...
        if isinstance(expected[key], dict):
            if not isinstance(actual[key], dict):
                return False
            if not _check_dict_has_keys(expected[key], actual[key], current_path):
                return False
    return True
...
class ToolBaseE2E:
    ...
    async def _run_test_with_expectations(
        ...
        file_name = (
            self.__class__.__name__.lower().replace("e2e", "").replace("test", "")
        )
        ...
        for i, tool_call in enumerate(response.tool_calls_made):
            ...
            assert (
                ...
                f"Should have called {tool_call.tool_name} tool with the correct parameters, but got: {tool_call.parameters}"
            )
            if test_expectations.tool_calls_expected[i].result != SHOULD_NOT_BE_EMPTY:
                ...
                if isinstance(expected_result, str):
                    assert expected_result in response.tool_results[i], (
                        f"Should have called {tool_call.tool_name} tool with the correct result, but got: {response.tool_results[i]}"
                    )
                else:
                    actual_result = json.loads(response.tool_results[i])
                    assert _check_dict_has_keys(expected_result, actual_result), (
                        f"Should have called {tool_call.tool_name} tool with the correct result structure, but got: {response.tool_results[i]}"
                    )
            else:
                assert len(response.tool_results[i]) > 0, (
                    f"Should have called {tool_call.tool_name} tool with non-empty result, but got: {response.tool_results[i]}"
                )
