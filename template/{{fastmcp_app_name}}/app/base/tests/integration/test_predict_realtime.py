...
class TestMCPRealtimePredictToolsIntegration:
    ...
    async def test_multiseries_regression(
        ...
        async with integration_test_mcp_session() as session:
            ...
            if response_dict["type"] == "inline":
                ...
                assert (
                    "_PREDICTION" in response_dict["data"]
                    or "sales" in response_dict["data"]
                )
            else:
    ...
    async def test_deployment_compatibility(self, timeseries_regression_project):
        ...
        # Note: Model object may not have target_type attribute, but we can verify it's a valid model
            ...
            for col in input_df.columns:
                if col != "sales":  # sales might not be in predictions
                    assert col in df.columns, (
                        f"Passthrough column {col} missing from output"
                    )
    ...
    async def test_predict_realtime_time_series_with_explanations(
        ...
        async with integration_test_mcp_session() as session:
            ...
            explanation_cols = [
                col for col in df.columns if "EXPLANATION" in col or "SHAP" in col
            ]
    ...
    async def test_predict_realtime_comprehensive_explanation_parameters(
        ...
        async with integration_test_mcp_session() as session:
            ...
            assert "temperature" in df.columns, (
                "Passthrough column 'temperature' missing"
            )
    ...
    async def test_predict_realtime_with_custom_endpoint_and_passthrough_all(
        ...
        async with integration_test_mcp_session() as session:
            ...
            for col in input_columns:
                if (
                    col != "sales"
                ):  # Target column may not be passed through or may be renamed
                    assert col in df.columns, (
                        f"Expected passthrough column {col} not found"
                    )

            # Test successful completion with passthrough columns
            print(
                f"Successfully tested passthrough_columns='all' with {len(df.columns)} total columns"
            )
