...
from app.base.shared.common import get_sdk_client
...
@pytest.fixture(scope="session")
def timeseries_regression_dataset_name():
    return "timeseries_regression_train.csv"


@pytest.fixture(scope="session")
def timeseries_regression_project_name():
...
def timeseries_regression_project(
    ...
    try:
        ...
        for deployment in deployments:
            if deployment.label.startswith(deployment_label):
    ...
    dataset = dr.Dataset.create_from_file(file_path=train_file)
...
@pytest.fixture(scope="session")
def multiseries_regression_dataset_name():
    return "multiseries_regression_train.csv"


@pytest.fixture(scope="session")
def multiseries_regression_project_name():
...
@pytest.fixture(scope="session")
def classification_dataset_name():
    return "text_classification_train.csv"


@pytest.fixture(scope="session")
def classification_project_name():
    return "MCP Test Text Classification Project"


@pytest.fixture(scope="session")
def classification_project(
    dr_client, test_data_dir, classification_dataset_name, classification_project_name
):
    ...
    for bp in blueprints:
        if any(
            keyword in bp.model_type.lower()
            for keyword in ["text", "tfidf", "word", "ngram"]
        ):
            text_blueprint = bp
            break
