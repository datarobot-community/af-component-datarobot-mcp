"""Unit tests for deployment info tools."""
...
async def test_get_deployment_info_success(mock_get_sdk_client):
    ...
    assert result_json["features"][0]["importance"] == 0.8
...
async def test_validate_prediction_data_error(mock_get_features):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
        ...
        assert (
            "Error in validate_prediction_data: JSONDecodeError: Expecting value"
...
async def test_validate_prediction_data_time_series_parse_error(
    ...
    mock_get_features.return_value = json.dumps(features_info)
    test_file = tmp_path / "test.csv"
    pd.DataFrame({"foo": [1, 2], "dt_col": ["bad", "bad"]}).to_csv(
...
async def test_generate_prediction_data_template_exception(mock_get_features):
    ...
    assert "Error in generate_prediction_data_template: Exception: fail" == str(
        exc_info.value
    )
...
async def test_validate_prediction_data_file_error(mock_get_features):
    ...
    assert (
        "Error in validate_prediction_data: FileNotFoundError: [Errno 2] No such file or directory: '/not/a/real/file.csv'"
        == str(exc_info.value)
    )
