...
async def test_predict_timeseries_regression_series_id_validation_error(
    ...
    assert (
        "Error in predict_realtime: ValueError: series_id_column 'invalid_column' not found in input data."
        == str(exc_info.value)
    )
...
async def test_predict_realtime_with_all_explanation_parameters(
    ...
    assert kwargs["threshold_high"] == 0.8
    assert kwargs["threshold_low"] == 0.2
    assert kwargs["explanation_algorithm"] == "shap"
    assert kwargs["timeout"] == 300
