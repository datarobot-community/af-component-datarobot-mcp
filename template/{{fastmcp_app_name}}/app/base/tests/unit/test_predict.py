...
@pytest.mark.asyncio
async def test_predict_with_deployment_by_file_path(patch_predict_dependencies):
    mock_job = MagicMock()
    mock_job.id = "jobid"
    mock_job.status = "COMPLETED"
    patch_predict_dependencies["mock_batch_job"].score.return_value = mock_job
    patch_predict_dependencies["mock_cred"].return_value = MagicMock(
        credential_id="cid"
    )
    result = await predict.predict_with_deployment_by_file_path("dep", "file.csv", 5)
...
@pytest.mark.asyncio
async def test_predict_with_deployment_by_ai_catalog(patch_predict_dependencies):
    with patch("app.base.tools.predict.uuid.uuid4", return_value="uuid"):
        mock_job = MagicMock()
        mock_job.id = "jobid"
        mock_job.status = "COMPLETED"
        patch_predict_dependencies["mock_batch_job"].score.return_value = mock_job
        result = await predict.predict_with_deployment_by_ai_catalog("dep", "dsid", 5)
        patch_predict_dependencies["mock_batch_job"].score.assert_called_once()
        assert "Finished Batch Prediction job ID jobid" in result


@pytest.mark.asyncio
async def test_predict_with_deployment_from_project_data(patch_predict_dependencies):
    with patch("app.base.tools.predict.uuid.uuid4", return_value="uuid"):
        mock_job = MagicMock()
        mock_job.id = "jobid"
        mock_job.status = "COMPLETED"
        patch_predict_dependencies["mock_batch_job"].score.return_value = mock_job
        result = await predict.predict_with_deployment_from_project_data(
            "dep", "pid", "dsid", "holdout", 5
        )
        patch_predict_dependencies["mock_batch_job"].score.assert_called_once()
        assert "Finished Batch Prediction job ID jobid" in result
...
@pytest.mark.asyncio
async def test_predict_with_deployment_by_file_path_timeout(patch_predict_dependencies):
    mock_job = MagicMock()
    mock_job.id = "jobid"
    ...
    with pytest.raises(MCPError) as exc_info:
        await predict.predict_with_deployment_by_file_path("dep", "file.csv", 1)
    assert (
        "Error in predict_with_deployment_by_file_path: AsyncTimeoutError: Job did not complete within the timeout period."
        == str(exc_info.value)
    )
...
async def test_predict_with_deployment_by_file_path_failure_error(
    ...
    with pytest.raises(MCPError) as exc_info:
        await predict.predict_with_deployment_by_file_path("dep", "file.csv", 1)
    assert (
        "Error in predict_with_deployment_by_file_path: AsyncFailureError: Job failed for some reason."
        == str(exc_info.value)
    )
...
async def test_predict_with_deployment_by_file_path_unsuccessful_error(
    ...
    with pytest.raises(MCPError) as exc_info:
        await predict.predict_with_deployment_by_file_path("dep", "file.csv", 1)
    assert (
        "Error in predict_with_deployment_by_file_path: AsyncProcessUnsuccessfulError: Job was unsuccessful."
        == str(exc_info.value)
    )


def test_get_or_create_s3_credential_create(monkeypatch):
    mock_cred = MagicMock()
...
def test_get_or_create_s3_credential_existing(monkeypatch):
    mock_cred = MagicMock()
...
@pytest.mark.asyncio
async def test_get_prediction_explanations_basic(monkeypatch):
    ...
    assert result["explanations"][0]["feature1"] == 0.5


@pytest.mark.asyncio
async def test_get_prediction_explanations_empty(monkeypatch):
    mock_model = MagicMock()
...
@pytest.mark.asyncio
async def test_get_prediction_explanations_sdk_error(monkeypatch):
    mock_model = MagicMock()
    mock_model.get_or_request_prediction_explanations = MagicMock(
