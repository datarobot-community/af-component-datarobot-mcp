...
async def test_get_model_info_from_deployment_not_found():
    with patch("app.base.tools.deployment.get_sdk_client") as mock_get_client:
        ...
        assert (
            "Error in get_model_info_from_deployment: Exception: 404 client error: {'message': 'Not Found'}"
            == str(exc_info.value)
        )
...
async def test_get_model_info_from_deployment_error():
    with patch(
        ...
        with pytest.raises(Exception) as exc_info:
            await deployment.get_model_info_from_deployment("dep_id")
        assert "Error in get_model_info_from_deployment: Exception: fail" == str(
            exc_info.value
        )
...
async def test_deploy_model_success():
    with patch("app.base.tools.deployment.get_sdk_client") as mock_get_client:
        ...
        mock_get_client.return_value = mock_client
        result_json = await deployment.deploy_model(
            "model123", "Test Deployment", "desc"
        )
