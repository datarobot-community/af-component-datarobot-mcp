...
from app.base.tools import training
...
async def test_suggest_use_cases():
    ...
    with patch("app.base.tools.training.get_sdk_client") as mock_get_client:
        ...
        assert any(
            s["problem_type"] == "Multiclass Classification" for s in suggestions
        )
...
async def test_start_autopilot_validation():
    ...
    with patch("app.base.tools.training.get_sdk_client") as mock_get_client:
        ...
        assert (
            "Error: Please provide either dataset_url or dataset_id, not both" in result
        )

        # Test missing target
        result = await training.start_autopilot(
            target="", dataset_url="http://test.com/data.csv"
        )
