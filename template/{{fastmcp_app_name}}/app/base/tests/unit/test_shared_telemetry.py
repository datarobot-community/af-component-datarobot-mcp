...
def test_initialize_telemetry_enabled() -> None:
    ...
    mock_credentials.datarobot.user_api_token = "test-token"
    ...
    with (
        ...
        mock_span.set_attribute.assert_called_with("custom.attr", "test-value")
        ...
        assert (
            ...
            == "X-DataRobot-Api-Key=test-token,X-DataRobot-Entity-Id=test-entity"
        )
...
async def test_trace_execution_async() -> None:
    ...
    with (
        ...
        mock_span.set_attribute.assert_any_call("custom.attr", "test-value")
