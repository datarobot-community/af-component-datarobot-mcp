...
class S3Config:
    def __init__(self) -> None:
        ...
        if not credentials.aws_access_key_id or not credentials.aws_secret_access_key:
            raise S3ConfigError(
                "AWS credentials not found. Please provide credentials or set environment variables."
            )
        ...
        try:
            ...
            try:
                ...
                self.client.put_object(
                    Bucket=self.bucket_name, Key=test_key, Body=b"test"
                )

                # Test LIST operation
                self.client.list_objects_v2(
                    Bucket=self.bucket_name, Prefix=test_key, MaxKeys=1
                )
                ...
                if error_code == "403":
                    raise S3ConfigError(
                        f"Access denied: Missing {operation} permissions for bucket {self.bucket_name}"
                    )
                else:
                    raise S3ConfigError(
            ...
            if error_code == "404":
                ...
                raise S3ConfigError(
                    f"Error accessing bucket {self.bucket_name}: {str(e)}"
                )
...
class MemoryResource(BaseModel):
    id: str
    memory_storage_id: Optional[str] = (
        None  # a memory resource can belong to a memory storage or it can be standalone act as a temp session memory
    )
...
class MemoryManager:
    ...
    def _get_resource_metadata_s3_key(
        ...
        if agent_identifier:
            if not storage_id:
                raise ValueError("Storage ID is required for agent memory scope")
            return f"agents/{agent_identifier}/storages/{storage_id}/resources/{resource_id}/metadata.json"
    ...
    def _handle_s3_error(
        operation: str, error: Exception, resource_id: Optional[str] = None
    ) -> None:
    ...
    def _validate_agent_identifier(self, agent_identifier: str) -> None:
        """Validate the agent identifier is a valid s3 string to be used as a key and the key does not already exist."""

        if not re.match(r"^[a-zA-Z0-9!-_.*\'()]+$", agent_identifier):
            raise ValueError(
                "Agent identifier must be a valid s3 string to be used as a key"
            )
    ...
    async def initialize_storage(
        ...
        try:
            s3_key = MemoryManager._get_storage_metadata_s3_key(
    ...
    async def delete_agent(self, agent_identifier: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted agent and all its memory storages from S3: {agent_identifier}"
            )
            return True
        except ClientError as e:
        ...
        try:
            ...
            if "Contents" in response:
                for obj in response["Contents"]:
                    ...
                    storage = MemoryStorage.model_validate_json(
    ...
    async def store_resource(
        ...
        memory_storage_id: Optional[str] = None,
        agent_identifier: Optional[str] = None,
        prompt: Optional[str] = None,
        tool_context: Optional[ToolContext] = None,
        embedding_vector: Optional[List[float]] = None,
        ...
        Args:
            ...
            agent_identifier: Required if memory_storage_id is provided, the agent that owns the storage
        ...
        if (agent_identifier and not memory_storage_id) or (
            ...
            raise ValueError(
                "Agent identifier and memory storage ID must be provided together"
            )
        ...
        try:
            ...
            if "Contents" in response:
                for obj in response["Contents"]:
                    ...
                    resource_data = json.loads(result["Body"].read().decode("utf-8"))
                    resource = MemoryResource.model_validate_json(
                        json.dumps(resource_data)
                    )

                    if (
                        memory_storage_id
                        and resource.memory_storage_id != memory_storage_id
                    ):
                        continue
        ...
        if "Contents" in response:
            for obj in response["Contents"]:
                if memory_storage_id in obj["Key"]:
                    # Extract agent identifier from key pattern: agents/{agent_id}/storages/{storage_id}/
                    parts = obj["Key"].split("/")
                    if len(parts) > 1:
    ...
    async def get_active_storage_id_for_agent(
        self, agent_identifier: str
    ) -> Optional[str]:
        """Get the active storage ID for an agent from S3."""
        try:
            key = self._get_active_storage_mapping_key(agent_identifier)
            result = self.s3_config.client.get_object(
                Bucket=self.s3_config.bucket_name, Key=key
            )
