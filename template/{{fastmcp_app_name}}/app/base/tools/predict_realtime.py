...
async def predict_with_deployment_by_ai_catalog_rt(
    ...
    """
    Make real-time predictions using a DataRobot deployment and an AI Catalog dataset using the datarobot-predict library.
    Use this for fast results when your data is not huge (not gigabytes). Results larger than 1MB will be returned as a resource id and S3 URL; smaller results will be returned inline as a CSV string.
    ...
    Returns:
        dict: {"type": "inline", "data": csv_str} for small results (<1MB), or {"type": "resource", "resource_id": ..., "s3_url": ...} for large results (>=1MB).
    """
...
async def predict_realtime(
    ...
    passthrough_columns: Optional[str] = None,
    ...
    try to infer or ask for a reasonable value, using frequent values or domain knowledge if available.
    ...
    Args:
        ...
        dataset: (Optional) CSV or JSON string representing the input data. If provided, this takes precedence over file_path.
        ...
        forecast_range_start: (Time Series) Start date for historical predictions (e.g., "2024-06-01").
    ...
    if series_id_column and series_id_column not in df.columns:
        raise ValueError(
            f"series_id_column '{series_id_column}' not found in input data."
        )
    ...
    is_time_series = bool(
        forecast_point or (forecast_range_start and forecast_range_end)
    )
