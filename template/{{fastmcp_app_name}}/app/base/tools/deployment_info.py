...
async def get_deployment_info(deployment_id: str) -> str:
    ...
    result = {
        ...
        "features": sorted(
            features, key=lambda x: (x.get("importance") or 0), reverse=True
        ),
...
async def generate_prediction_data_template(deployment_id: str, n_rows: int = 1) -> str:
    ...
    - When using this tool, always consider feature importance. For features with high importance, try to infer or ask for a reasonable value, using frequent values or domain knowledge if available. For less important features, you may leave them blank.
    - If frequent values are available for a feature, they will be used as sample values; otherwise, blank fields will be used.
      Please note that using frequent values in your predictions data can influence the prediction, think of it as sending in the average value for
      the feature.  If you don't want this effect on your predictions leave the field blank you in predictions dataset.
    ...
    result += (
        f"# Target: {features_info['target']} (Type: {features_info['target_type']})\n"
    )
    ...
    if "time_series_config" in features_info:
        ...
        if ts["series_id_columns"]:
            result += (
                f"# Multiseries ID Columns: {', '.join(ts['series_id_columns'])}\n"
            )
...
async def validate_prediction_data(
    ...
    for feature in required_features:
        ...
        if feature_name not in data_columns:
            if feature.get("importance", 0) > 0.1:
                validation_report["warnings"].append(
                    f"Missing important feature: {feature_name} (importance: {feature.get('importance', 0):.2f})"
                )
            else:
                validation_report["warnings"].append(
                    f"Missing feature column: {feature_name} (column will be treated as missing values)"
                )
            continue
    ...
    expected_features = {
        f["name"] if "name" in f else f["feature_name"]
    ...
    if "time_series_config" in features_info:
        ...
        if ts_config["datetime_column"] not in data_columns:
            ...
            # Verify it can be parsed as datetime (skip if all missing)
            if (
...
async def get_deployment_features(deployment_id: str) -> str:
    ...
    if not info_json.strip().startswith("{"):
        ...
        return json.dumps(
            {"features": [], "total_features": 0, "error": info_json}, indent=2
        )
