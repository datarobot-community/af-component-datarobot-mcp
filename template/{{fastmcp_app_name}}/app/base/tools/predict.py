...
def _handle_prediction_resource(job: Any, bucket: str, key: str, deployment_id: str, input_desc: str) -> str:
    ...
    return (
        ...
        f"Job status: {job.status} and you can find the job on the DataRobot UI at /deployments/batch-jobs. "
    )
...
async def predict_with_deployment_by_file_path(
    ...
    """
    Make predictions using a DataRobot deployment and a local CSV file using the DataRobot Python SDK. Use this tool to
    score large amounts of data, for small amounts of data use the predict_with_deployment_by_file_path_rt tool.
    Args:
...
async def predict_with_deployment_by_ai_catalog(
    ...
    """
    Make predictions using a DataRobot deployment and an AI Catalog dataset using the DataRobot Python SDK.
    Args:
        deployment_id: The ID of the DataRobot deployment to use for prediction.
...
async def predict_with_deployment_from_project_data(
    ...
    """
    Make predictions using a DataRobot deployment using the training data associated with the project that created the deployment.
    Use this tool to score holdout, validation, or allBacktest partitions of the training data.
    Can request a specific partition of the data, or use an external dataset (with dataset_id) stored in AI Catalog.
    Args:
        ...
        dataset_id: (Optional) The ID of the external dataset, ususally stored in AI Catalog, to use for prediction.
        partition: (Optional)The partition of the DataRobot dataset to use for prediction, could be 'holdout', 'validation', or 'allBacktest'.
