...
async def analyze_dataset(dataset_id: str) -> str:
    ...
    potential_targets = _identify_potential_targets(
...
def _analyze_target_for_use_cases(
        ...
        if unique_count == 2:
            ...
            suggestions.append(
                UseCaseSuggestion(
                    ...
                    confidence=0.8,
                    reasoning=f"Column {target_col} has exactly 2 unique values, suggesting binary classification",
                )
            )
        ...
        if unique_count == 2:
            ...
            suggestions.append(
                UseCaseSuggestion(
                    ...
                    problem_type="Multiclass Classification",
                    confidence=0.8,
                    reasoning=f"Column {target_col} is categorical with {unique_count} manageable classes",
                )
            )
...
async def start_autopilot(
    ...
    project_name: Optional[str] = "MCP Project",
) -> str:
    """
    ...
    Args:
        ...
        dataset_url: Optional, The URL to the dataset to upload (optional if dataset_id is provided) for a new project.
        dataset_id: Optional, The ID of an existing dataset in AI Catalog (optional if dataset_url is provided) for a new project.
    ...
    if not project_id:
        ...
        project = client.Project.create_from_dataset(
            dataset.id, project_name=project_name
        )
    else:
...
async def get_model_roc_curve(
    ...
    Args:
        ...
        source: The source of the data to use for the ROC curve ('validation' or 'holdout' or 'crossValidation')
    Returns:
        JSON string containing:
            ...
            "roc_points": [
                {
                    ...
                    "positive_predictive_value": point.get(
                    ...
                    "negative_predictive_value": point.get(
...
async def get_model_feature_impact(project_id: str, model_id: str) -> str:
    ...
    return json.dumps(
        {"data": feature_impact, "ui_panel": ["feature-impact"]}, indent=2
    )
...
async def get_model_lift_chart(
    ...
    Args:
        project_id: The ID of the DataRobot project
        model_id: The ID of the model to analyze
        source: The source of the data to use for the lift chart ('validation' or 'holdout' or 'crossValidation')
    Returns:
        JSON string containing:
